...
def script_it():
    ...
    def did_curse_monk(gs):
        return 'CRUCIFIX_STOLEN' in gs.hidden_triggers and gs.weapon == 1
    def did_not_curse_monk(gs):
        return 'CRUCIFIX_STOLEN' not in gs.hidden_triggers and gs.weapon == 1
    ...
    def _examine_forest_well(game_view, _GameView):
        ...
        if 'EMPTY_BOTTLE' in gs.items:
            if 'HOLY_WELL' in gs.hidden_triggers:
                ...
                new_game_state = gs._replace(message='You fill the bottle\nwith water.',
                                             msg_place=MessagePlacement.UP)
            if 'FULL_BOTTLE' not in gs.hidden_triggers:
                new_game_state = new_game_state.with_hidden_trigger('FULL_BOTTLE')
    ...
    mapscript_add_boulder(trigger_pos=(10, 2, 12), start_at=(10, 2, 13), _dir='north', message="You hear a loud thud.\nSomething is approaching\n")
    ...
    mapscript_add_message((7, 2, 2), 'You might want to pray\non the grave of the\ntemplar commander.\nMay he rest in peace.', extra_render=render_grave_icon)  # hint on sign in the water
        ...
        if gs.facing == 'north' and not gs.tile_override_at((8,8,11)): #mirror not taken yet
            ...
            game_view.actions['TAKE_MIRROR'] = _GameView(gs
                ...
                ._replace(items=gs.items + ('HAND_MIRROR',), treasure_id=41,
                          message="You remove the mirror\n\nA secret passage opens!", msg_place=MessagePlacement.UP))
